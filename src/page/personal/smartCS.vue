      ...
      <el-tab-pane class="lately" label="最近用过的" name="first">
        ...
        <ul v-if="!nohistory">
          <li v-for="(allhistory , index) in allhistorys" :key="index" @click="gotoapplication(allhistory, index)">
            <div class="logoicon"><img src="static/images/login.png" alt=""></div>
...
export default {
    ...
    userType: {
      handler: function (val, old) {
        ...
        if (!val || val.length == 0) {
          ...
          self.currentRole = self.vuexcurrentRole.roleCode;
        }
      },
      deep: true
    }
  ...
  methods: {
    ...
    async gotoapplication(value, index) {
      ...
      this.$store.dispatch("setHeaderTitle", value.menu_name)
      /* let self = this;
      let operationdata = {
        userId: self.userId,
        menu_code: menucode,
